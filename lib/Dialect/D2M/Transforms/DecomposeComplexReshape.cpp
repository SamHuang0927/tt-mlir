...
// Check if the reshape only moves singleton (size-1) dimensions and can be
// decomposed into a reshape + permute (see decomposePurePermute).
//
...
// Check if singleton dimensions need to move relative to non-singleton dims.
// Detects cases like (128, 4, 1) -> (1, 512) where a trailing 1 must become
// a leading 1, requiring a permute before the reshape.
//
// Examples:
...
enum class DecomposeType { PurePermute, SingletonPermute };
...
public:
  ...
  void runOnOperation() final {
    ...
    for (auto [op, type] : opsToProcess) {
      ...
      if (type == DecomposeType::PurePermute) {
        decomposePurePermute(rewriter, op);
      } else {
        decomposeSingletonPermute(rewriter, op);
      }
    }
  }

private:
  // Decompose a pure permute into at most one reshape + one last-two-dim swap.
  ...
  // Decompose a reshape where trailing 1s become leading 1s with a genuine
  // reshape.  Strategy: collapse to (X, 1), swap to (1, X), reshape to output.
