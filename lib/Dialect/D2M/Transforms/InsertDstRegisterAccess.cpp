...
static Value getBlockingLoopIV(Operation *op, int64_t dim, OpBuilder &rewriter,
  ...
  while (current) {
    if (auto scfFor = dyn_cast<scf::ForOp>(current)) {
...
public:
  ...
  LogicalResult matchAndRewrite(TileReduceOp op,
    ...
    if (reduceDim == ReduceDim::R) {
      Value iterIndex =
          getBlockingLoopIV(op, static_cast<int64_t>(1), rewriter, op.getLoc());
      ...
      Value iterIndex =
          getBlockingLoopIV(op, static_cast<int64_t>(0), rewriter, op.getLoc());
      auto condOp = rewriter.create<arith::CmpIOp>(
          op.getLoc(), arith::CmpIPredicate::ne, iterIndex,
