...
func.func @distributed_rms_norm_with_weight(%arg0: tensor<1x1x32x128xbf16>, %weight: tensor<128xbf16>) -> tensor<1x1x32x128xbf16> {
  ...
  %w = "ttir.mesh_shard"(%weight) <{shard_dims = array<i64: -1, 0>, shard_direction = #ttcore.shard_direction<full_to_shard>, shard_shape = array<i64: 2>, shard_type = #ttcore.shard_type<devices>}> : (tensor<128xbf16>) -> tensor<64xbf16>

  // Distributed RMS norm with weight + all-gather
  %1 = "ttir.distributed_rms_norm"(%0, %w) <{cluster_axis = 1 : ui32, epsilon = 1.000000e-05 : f32}> : (tensor<1x1x32x64xbf16>, tensor<64xbf16>) -> tensor<1x1x32x128xbf16>
  // CHECK: "ttnn.distributed_rms_norm"
